#!/usr/bin/env python3
"""EIGRP troubleshooting with stable config baseline"""
...
def check_eigrp_interface_timers(tn, device_name):
    ...
    eigrp_intf_output = get_eigrp_interfaces(tn)
    if not eigrp_intf_output:
        return []
    
    for line in eigrp_intf_output.split('\n'):
        ...
        pattern = rf'{re.escape(intf_name)}.*?Hello.*?(\d+).*?Hold.*?(\d+)'
        match = re.search(pattern, eigrp_intf_output, re.IGNORECASE | re.DOTALL)
        
        if match:
            current_hello = int(match.group(1))
            current_hold = int(match.group(2))
            expected_hello = 5
            expected_hold = 15
