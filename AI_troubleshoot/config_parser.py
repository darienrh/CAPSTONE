#!/usr/bin/env python3
"""Simple config parser that reads latest stable config"""
...
def parse_device_config(device_name, config):
    ...
    if ospf_match:
        ...
        if rid_match:
            info['ospf']['router_id'] = rid_match.group(1)
    ...
    for intf_name, intf_config in interface_sections:
        ...
        if hello_match or dead_match:
            intf_info['ospf_hello'] = int(hello_match.group(1)) if hello_match else 10
            intf_info['ospf_dead'] = int(dead_match.group(1)) if dead_match else 40
        ...
        if eigrp_hello_match or eigrp_hold_match:
            intf_info['eigrp_hello'] = int(eigrp_hello_match.group(1)) if eigrp_hello_match else 5
            intf_info['eigrp_hold'] = int(eigrp_hold_match.group(1)) if eigrp_hold_match else 15
...
def should_interface_be_up(device_name, interface):
    ...
    return not intf_info.get('shutdown', False)
