...
def telemetry_stream_thread(gns3_url, device_names):
    ...
    while _telemetry_active:
        try:
            bps_query = '''
                SELECT non_negative_derivative(mean("ifInOctets"), 1s) * 8 AS bps_in,
                       non_negative_derivative(mean("ifOutOctets"), 1s) * 8 AS bps_out,
                       mean("ifInErrors") AS err_in,
                       mean("ifOutErrors") AS err_out
                FROM "interface"
                WHERE time > now() - 60s
                GROUP BY "hostname", time(10s)
...
if __name__ == '__main__':
    ...
    print(f"  Open http://192.168.231.10:5000 to access from host PC")
    print("=" * 60)
