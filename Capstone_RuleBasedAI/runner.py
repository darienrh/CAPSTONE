...
def include_gns3_node(node):
    ...
    if name.startswith(("switch", "pc", "kali", "cloud")):
        return False
...
class DiagnosticRunner:
    ...
    def connect_to_devices(self, device_names):
        ...
        def connect_one(device_name):
            ...
            tn = connect_device(console_port, host=self.gns3_host)
    ...
    def print_completion_summary(self):
        ...
        for result in fix_results:
            ...
            if rule_id:
                ...
                self.reporter.print_info(f"[Learning] Rule {rule_id} {'succeeded' if success else 'failed'} "
                      f"for {problem_type} on {result['device']}")
            else:
                self.reporter.print_info(f"[Learning] No matching rule for {problem_type} ({category}) "
                      f"on {result['device']}")
    ...
    def _run_post_session_learning(self):
        ...
        if history_len >= RuleMiner.MIN_OCCURRENCES:
            ...
            if added:
                self.reporter.print_success(f"[RuleMiner] {added} new rule(s) added to KB from this session's data")
        else:
            self.reporter.print_dim(f"[RuleMiner] Skipping mining — need {RuleMiner.MIN_OCCURRENCES}+ history entries "
                  f"(have {history_len})")

    def show_explanation_traces(self):
        traces = self.inference_engine.get_explanation_traces()
        if not traces:
            self.reporter.print_info("No explanation traces recorded this session.")
            return
...
def main():
    ...
    if Confirm.ask("Save stable configurations of all routers now?"):
        runner.save_stable_configurations(available_devices)
