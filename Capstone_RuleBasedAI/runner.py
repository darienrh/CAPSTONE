...
class DiagnosticRunner:
    ...
    def print_completion_summary(self):
        ...
        for result in fix_results:
            ...
            # Look up matching rule_id by type+category against KB rules
            rule_id = self.knowledge_base.get_rule_id_for_problem(problem_type, category)
            ...
            # Update rule confidence directly
            if rule_id:
                ...
                print(f"[Learning] No matching rule found for {problem_type} "
                    f"({category}) on {result['device']}")
...
def run():
    """Entry point that can be called programmatically"""
    try:
        main()
