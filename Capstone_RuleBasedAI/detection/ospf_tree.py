...
def check_passive_interfaces(config, device_name, config_manager=None):
    ...
    for line in config.split('\n'):
        ...
        if line_stripped.startswith('router ospf'):
            ...
            continue
        if in_ospf_section and (line_stripped.startswith('!') or
                                  line_stripped.startswith('router ') or
                                  line_stripped.startswith('interface ')):
            in_ospf_section = False
            continue
        if in_ospf_section and 'passive-interface' in line_stripped.lower():
            match = re.search(r'passive-interface\s+(\S+)', line_stripped, re.IGNORECASE)
            if match:
...
def check_network_statements(config, device_name, config_manager=None):
    ...
    for net, wild, area in extra:
        # Find what the correct area should be from baseline
        correct_area = area  # fallback
        for n in expected_networks:
            if n['network'] == net and n['wildcard'] == wild:
                correct_area = n['area']
                break
        issues.append({
            'type': 'extra network',
...
def get_ospf_fix_commands(issue_type, issue_details, device_name, config_manager=None):
    ...
    if issue_type in ['hello interval mismatch', 'dead interval mismatch', 'ospf timer mismatch']:
        ...
        wildcard = issue_details.get('wildcard')
        area = issue_details.get('area')
        correct_area = issue_details.get('correct_area', area)

        commands = [
            f"router ospf {process_id}",
            f"no network {network} {wildcard} area {area}",
        ]
        # If the network belongs in a different area, restore it correctly
        if correct_area != area:
            commands.append(f"network {network} {wildcard} area {correct_area}")
        commands.append("end")
        return commands
