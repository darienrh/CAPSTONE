...
def check_network_statements(tn, device_name):
    ...
    FIXED: Now properly handles baseline comparison
    """
    config = get_eigrp_config(tn)
    if not config:
        return []
    ...
    try:
        from core.config_manager import ConfigManager
        cm = ConfigManager()
        baseline = cm.get_device_baseline(device_name)
    except:
        baseline = get_device_baseline(device_name)
    
    # Get expected networks from baseline
    ...
    # Get current networks from running config
    ...
    for line in config.split('\n'):
        ...
        # Check if we're entering EIGRP config section
        if line.startswith('router eigrp'):
            in_eigrp_section = True
            continue
        
        # Check if we're leaving EIGRP section
        if in_eigrp_section and line.startswith('!'):
            in_eigrp_section = False
            continue
        
        # Only process network statements within EIGRP section
    ...
    # If baseline has no networks defined, don't flag anything as extra
    # This prevents false positives when baseline is empty or not loaded
    if not expected_networks:
        print(f"[WARNING {device_name}] No expected networks in baseline - skipping network check")
        return []
    
    # Missing networks (in baseline but not in current config)
    ...
    # Extra networks (in current config but not in baseline)
...
def troubleshoot_eigrp(device_name, tn, auto_prompt=True):
    ...
    # FIXED: Now calling check_eigrp_interface_timers to detect timer mismatches
