...
class DiagnosticRunner:
    ...
    def restore_stable_configurations(self, device_names=None):
        ...
        def restore_single_device(device_name, config):
            try:
                ...
                commands.append('\x03')  # Ctrl+C
                commands.append('configure terminal')
                
                # Split config into commands
                for line in config.split('\n'):
                    ...
                    if any(skip in line.lower() for skip in ['version', 'hostname', 'service ', 'enable ', 'line ']):
                        continue
                ...
                for cmd in commands:
                    ...
                    time.sleep(0.05)  # Very short delay between commands
    ...
    def print_completion_summary(self):
        ...
        for result in fix_results:
            problem = {'device': result['device'], 'type': 'fix_applied'}
            solution = {'commands': result['commands'], 'verification': result['verification']}
            self.knowledge_base.add_problem_solution_pair(problem, solution, success=True)
