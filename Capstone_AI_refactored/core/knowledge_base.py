...
class KnowledgeBase:
    ...
    def _validate_and_format_rule(self, rule, problem_dict, baseline):
        """
        IMPROVED: Validate rule AND format commands with baseline values
        ...
        Returns:
            Rule with updated commands and validation status
        """
        validated_rule = rule.copy()
        ...
        # ===== OSPF VALIDATIONS =====
        if 'ospf timer' in problem_type or 'hello interval' in problem_type or 'dead interval' in problem_type:
            if interface in baseline.get('interfaces', {}):
                intf_baseline = baseline['interfaces'][interface]
        ...
        # ===== FORMAT COMMANDS WITH BASELINE VALUES =====
        if is_valid and baseline_values:
            # Merge problem_dict with baseline_values (baseline takes precedence)
