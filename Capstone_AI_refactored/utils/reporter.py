...
HISTORY_DIR = Path.home() / "Capstone_AI_refactored" / "history" / "runs"
...
class Reporter:
    ...
    def print_scan_summary(self, detected_issues):
        ...
        """
        if not detected_issues.get('interfaces') and not detected_issues.get('eigrp') and not detected_issues.get('ospf'):
