...
from config_parser import is_eigrp_router, is_ospf_router, load_latest_stable_config
...
console = Console()
HISTORY_DIR = Path.home() / "history" / "runs"
...
def restore_device_config(tn, device_name, config_content):
    """Restore a device configuration from file content"""
    try:
        commands = [
            b'\r\n', b'\x03', b'enable\r\n',
            b'configure terminal\r\n',
            b'config replace flash:backup.cfg force\r\n'  # This won't work, we need different approach
        ]
        
        # Better approach: Parse and apply config line by line
...
class DiagnosticRunner:
    ...
    def restore_stable_configurations(self, device_names=None):
        """Restore configurations from last stable baseline"""
        console.print("\n[bold cyan]RESTORING STABLE CONFIGURATIONS[/bold cyan]")
        
        # Load baseline configs
        if not load_latest_stable_config():
            console.print("[red]No stable configuration file found![/red]")
            return False
        
        # Read the latest stable config file
        config_files = list(CONFIG_DIR.glob("config_stable*.txt"))
        if not config_files:
            console.print("[red]No stable configuration files found![/red]")
            return False
        
        config_files.sort(key=lambda x: x.stat().st_mtime, reverse=True)
        latest_config = config_files[0]
        
        console.print(f"[yellow]Using baseline: {latest_config.name}[/yellow]")
        ...
        if not devices_to_restore:
            console.print("[red]No matching devices found in baseline![/red]")
            return False
        
        console.print(f"[yellow]Will restore: {', '.join(devices_to_restore)}[/yellow]")
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"),
            ...
            task = progress.add_task("[cyan]Restoring configurations...", total=len(devices_to_restore))
            
            for device_name in devices_to_restore:
                progress.update(task, description=f"[cyan]Restoring {device_name}...")
                
                console_port = self.nodes.get(device_name)
                if not console_port:
                    console.print(f"[red]Could not connect to {device_name}[/red]")
                    progress.advance(task)
                    continue
                ...
                if restore_device_config(tn, device_name, config):
                    console.print(f"[green]✓ Restored {device_name}[/green]")
                else:
                    console.print(f"[red]✗ Failed to restore {device_name}[/red]")
        ...
        for device, problems in detected_issues['interfaces'].items():
            ...
            for problem in problems:
                ...
                if problem_type == 'ip address mismatch':
                    ...
                    if auto_fix_all or Confirm.ask("Fix IP address?"):
                    ...
                    console.print(f"  Expected: {problem['expected_ip']} {problem['expected_mask']}")
                    if auto_fix_all or Confirm.ask("Configure IP address?"):
                        from interface_tree import fix_interface_ip
                        if fix_interface_ip(tn, interface, problem['expected_ip'], problem['expected_mask']):
                    ...
                    if auto_fix_all or Confirm.ask("Apply 'no shutdown'?"):
        ...
        for device, problems in detected_issues['eigrp'].items():
            ...
            for issue in problems:
                ...
                if not fix_commands:
                    ...
                    continue
                if auto_fix_all or Confirm.ask(f"Apply fix?"):
...
def main():
    ...
    # Ask about reverting to baseline
    if Confirm.ask("Revert configs to last stable version?", default=False):
        ...
        if revert_mode == "all":
            ...
            device_input = console.input("Enter devices to revert (e.g. 'R1, R2, R4'): ").strip()
            if device_input:
